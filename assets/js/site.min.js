!(function(i) {
          ...
          var s = a
            .reduce(function(t, e) {
              return (
                t.push(
                  l
                    .replace(/{{name}}/g, _.escape(e[0]))
                    .replace(/{{description}}/g, _.escape(e[1]))
                    .replace(/{{stars}}/g, _.escape(e[2]))
                ),
                t
              );
            }, [])
      ...
      .on('click', '.onX', function() {
        i(this)
          ...
          i(this)
            .removeClass('fa-close')
            .addClass('fa-search'),
          i('#common-packages').show(),
          i('#search-results').hide();
      }),
    i(window).scroll(function() {
      100 < i(this).scrollTop() ? i('#back-to-top').fadeIn() : i('#back-to-top').fadeOut();
    }),
    i('#back-to-top').on('click', function(t) {
      return t.preventDefault(), i('html, body').animate({ scrollTop: 0 }, 100), !1;
    });

  // 给header标签添加base64背景图片;
  /*
  try {
    if ('/' !== window.location.pathname) {
      var t = i('.site-header h1')
        .first()
        .text();
  ...
  i('.btn-group .btn').click(function() {
    ...
    console.log('Protocal ', t, ' was selected'),
      'relative' === t
        ? i('.library-url').each(function() {
            var t = i(this).text();
            i(this).text(t.replace(/^https?:/i, '')),
              i(this)
                .parent()
                .removeClass('js-https');
          })
        : 'http' === t
        ? i('.library-url').each(function() {
            var t = i(this).text();
  ...
  $(function() {
    $('.library-url').each(function() {
      ...
      /\.css$/.test(t)
        ? $(this).before(
            '<div class="zero-clipboard"><span class="btn-clipboard btn-clipboard-code" title="复制到剪贴板"><i class="fa fa-code"></i> 复制 &lt;link&gt; 标签</span><span class="btn-clipboard btn-clipboard-link" title="复制到剪贴板"><i class="fa fa-link"></i> 复制链接</span></div>'
          )
        : /\.js$/.test(t)
        ? $(this).before(
            '<div class="zero-clipboard"><span class="btn-clipboard btn-clipboard-code" title="复制到剪贴板"><i class="fa fa-code"></i> 复制 &lt;script&gt; 标签</span><span class="btn-clipboard btn-clipboard-link" title="复制到剪贴板"><i class="fa fa-link"></i> 复制链接</span></div>'
          )
        : $(this).before(
            '<div class="zero-clipboard"><span class="btn-clipboard btn-clipboard-link" title="复制到剪贴板"><i class="fa fa-link"></i> 复制链接</span></div>'
          );
    }),
      $('.btn-clipboard').tooltip({ container: 'body' });
    var t = new Clipboard('.btn-clipboard', {
      text: function(t) {
        var e = $(t),
          ...
          s = (url = a.text());
        return (
          e.hasClass('btn-clipboard-code') &&
            (/\.css$/.test(url)
              ? (s = '<link href="' + url + '" rel="stylesheet">')
              : /\.js$/.test(url) && (s = '<script src="' + url + '"></script>')),
          s
        );
      }
    });
